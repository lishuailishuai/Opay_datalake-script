...
$(mysql -h${HOST_RD} -u${USER_RD} -P${PORT_RD} -p${PASS_RD} oride_data --skip-column-names -e"
    ...
    select
        ...
        if(sum(if(o.status=4 or o.status=5,1,0))>0, round(sum(if(o.pickup_time>0,
            o.pickup_time-o.take_time, 0))/60/sum(if(o.status=4 or o.status=5,1,0)), 1), 0) as avg_take,
