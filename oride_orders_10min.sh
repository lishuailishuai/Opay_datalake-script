...
while read day hour drivers; do
    ...
    $(mysql  -h${HOST_RD} -u${USER_RD} -P${PORT_RD} -p${PASS_RD} oride_data --skip-column-names -e"select date_format(now(), '%Y-%m-%d'), date_format(now(), '%k'), count(1) from data_driver_extend where serv_mode>0")
_feof