...
for db_name, table_name, conn_id, prefix_name,priority_weight_nm in table_list:
    ...
    import_table = BashOperator(
        ...
        bash_command='''
            ...
            --username {username} \
            --password {password} \
            --query 'select * from {table} where ((FROM_UNIXTIME(UNIX_TIMESTAMP(create_time), "%Y-%m-%d %H") between "{{{{ ds }}}} 01" and "{{{{ macros.ds_add(ds, +1) }}}} 00") OR (FROM_UNIXTIME(UNIX_TIMESTAMP(update_time), "%Y-%m-%d %H") between "{{{{ ds }}}} 01" and "{{{{ macros.ds_add(ds, +1) }}}} 00")) AND $CONDITIONS' \
            --split-by id \
