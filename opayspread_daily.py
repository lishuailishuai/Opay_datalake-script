...
FROM opay_spread.promoter_user as p JOIN 
    ...
    FROM oride_dw_ods.ods_sqoop_base_data_order_df as o JOIN opay_spread.rider_signups as r
...
FROM opay_spread.promoter_user as p JOIN 
    ...
    FROM opay_spread.rider_signups as r JOIN oride_dw_ods.ods_sqoop_base_data_driver_extend_df as d
    ON r.driver_id = d.id 
    WHERE 
...
(
	...
	from
	(
		select * from oride_dw_ods.ods_sqoop_base_data_order_df
		where dt = '{ds}'
		and from_unixtime(create_time, 'yyyy-MM-dd') = '{ds}' 
	) a
	left outer join
	(
		select * from oride_dw_ods.ods_sqoop_base_data_driver_comment_df where dt = '{ds}'
	) b on a.id = b.order_id
	left outer join
	(
		select * from oride_dw_ods.ods_sqoop_base_data_order_payment_df where dt = '{ds}' and mode>=0
	) c on a.id = c.id
	group by a.driver_id
) TA
LEFT JOIN 
(
    select * from oride_dw_ods.ods_sqoop_base_data_driver_df where dt = '{ds}'
) TB on TA.driver_id = TB.id
LEFT JOIN 
(
    select * from oride_dw_ods.ods_sqoop_base_data_driver_extend_df where dt = '{ds}'
