...
dag = airflow.DAG('dwd_otrade_b2b_otrade_order_item_hi',
...
##----------------------------------------- 变量 ---------------------------------------##
db_name = "otrade_dw"
table_name = "dwd_otrade_b2b_otrade_order_item_hi"
...
##----------------------------------------- 依赖 ---------------------------------------##
### 检查当前小时的分区依赖
###oss://opay-datalake/otrade_all_hi/ods_binlog_base_otrade_order_item_all_hi
ods_binlog_base_otrade_order_item_all_hi_check_task = OssSensor(
        task_id='ods_binlog_base_bd_admin_users_all_hi_check_task',
        bucket_key='{hdfs_path_str}/dt={pt}/hour={hour}/_SUCCESS'.format(
            hdfs_path_str="otrade_all_hi/ods_binlog_base_otrade_order_item_all_hi",
            pt='{{ds}}',
...
def dwd_otrade_b2b_otrade_order_item_hi_sql_task(ds, v_date):
    HQL = '''
...
--1.最后将去重的结果集插入到表中
insert overwrite table otrade_dw.dwd_otrade_b2b_otrade_order_item_hi partition(country_code,dt,hour)
select
  id
  ,zorder_id
  ,order_id
...
def execution_data_task_id(ds, dag, **kwargs):
   ...
   # 读取sql
    _sql = "\n" + cf.alter_partition() + "\n" + dwd_otrade_b2b_otrade_order_item_hi_sql_task(ds, v_date)
...
dwd_otrade_b2b_otrade_order_item_hi_task = PythonOperator(
    task_id='dwd_otrade_b2b_otrade_order_item_hi_task',
...
ods_binlog_base_otrade_order_item_all_hi_check_task >> dwd_otrade_b2b_otrade_order_item_hi_task
