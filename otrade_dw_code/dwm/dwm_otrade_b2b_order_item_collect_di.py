...
dag = airflow.DAG('dwm_otrade_b2b_order_item_collect_di',
...
##----------------------------------------- 变量 ---------------------------------------##
db_name = "otrade_dw"
table_name = "dwm_otrade_b2b_order_item_collect_di"
...
dwm_otrade_b2b_order_item_collect_hi_task = OssSensor(
    task_id='dwm_otrade_b2b_order_item_collect_hi_task',
    bucket_key='{hdfs_path_str}/country_code=NG/dt={pt}/hour={hour}/_SUCCESS'.format(
        hdfs_path_str="otrade/otrade_dw/dwm_otrade_b2b_order_item_collect_hi",
        pt='{{ds}}',
        hour='23'
    ),
...
def dwm_otrade_b2b_order_item_collect_di_sql_task(ds):
    HQL = '''
...
--1.最后将去重的结果集插入到表中
insert overwrite table otrade_dw.dwm_otrade_b2b_order_item_collect_di partition(country_code,dt)
select
  id
  ,zorder_id
  ,order_id
...
from
  ...
  from
    otrade_dw.dwm_otrade_b2b_order_item_collect_hi
  where
    dt = '{pt}'
  ) as a
where
...
def execution_data_task_id(ds, dag, **kwargs):
    ...
    # 读取sql
    # _sql = dwm_otrade_b2b_order_item_collect_di_sql_task(ds)
    ...
    # 读取sql
    _sql = "\n" + cf.alter_partition() + "\n" + dwm_otrade_b2b_order_item_collect_di_sql_task(ds)
...
dwm_otrade_b2b_order_item_collect_di_task = PythonOperator(
...
dwm_otrade_b2b_order_item_collect_hi_task >> dwm_otrade_b2b_order_item_collect_di_task
