...
dag = airflow.DAG('dim_oride_passenger_base_dev',
...
SELECT t1.passenger_id,
       ...
       from_unixtime(t2.local_register_time,'yyyy-MM-dd HH:mm:ss') AS register_time,
       -- 注册时间

       from_unixtime(t2.local_login_time,'yyyy-MM-dd HH:mm:ss') AS login_time,
       -- 最后登陆时间
...
LEFT OUTER JOIN
  (SELECT passenger_id,
          ...
          (register_time + 1 * 60 * 60 * 1) as local_register_time,
          
          (login_time + 1 * 60 * 60 * 1) as local_login_time
