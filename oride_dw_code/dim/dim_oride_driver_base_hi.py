...
dag = airflow.DAG('dim_oride_driver_base_hi',
...
db_name = "oride_dw"
table_name = "dim_oride_driver_base_hi"
hdfs_path = "oss://opay-datalake/oride/oride_dw/" + table_name
##----------------------------------------- 依赖 ---------------------------------------##


dwd_oride_driver_df_prev_day_task = OssSensor(
        task_id='dwd_oride_driver_df_prev_day_task',
...
dwd_oride_driver_extend_df_hi_prev_day_task = OssSensor(
        task_id='dwd_oride_driver_extend_df_hi_prev_day_task',
        bucket_key='{hdfs_path_str}/dt={pt}/_SUCCESS'.format(
            hdfs_path_str="oride/oride_dw/dwd_oride_driver_extend_df_hi/country_code=nal",
            pt='{{ds}}',
            now_day='{{macros.ds_add(ds, +1)}}',
            hour='{{ execution_date.strftime("%H") }}'
        ),
...
def dim_oride_driver_base_hi_sql_task(ds):
    HQL = '''
    ...
    (
        ...
        SELECT *
            FROM oride_dw.dwd_oride_driver_extend_df_hi
        where dt='{pt}' 
    ...
    ON dri.driver_id=ext.driver_id
...
def execution_data_task_id(ds, **kargs):
    ...
    # 读取sql
    _sql = dim_oride_driver_base_hi_sql_task(ds)
...
dim_oride_driver_base_hi_task = PythonOperator(
    task_id='dim_oride_driver_base_hi_task',
    python_callable=execution_data_task_id,
    provide_context=True,
    dag=dag
)

dwd_oride_driver_df_prev_day_task >> dim_oride_driver_base_hi_task
dwd_oride_driver_extend_df_hi_prev_day_task >> dim_oride_driver_base_hi_task
dim_oride_city_prev_day_tesk >> dim_oride_driver_base_hi_task
