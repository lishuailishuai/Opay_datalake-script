...
dag = airflow.DAG( 'dwd_oride_finance_driver_repayment_extend_test_df', 
...
table_name="dwd_oride_finance_driver_repayment_extend_test_df"
hdfs_path="ufile://opay-datalake/oride/oride_dw/"+table_name

##----------------------------------------- 脚本 ---------------------------------------## 

dwd_oride_finance_driver_repayment_extend_test_df_task = HiveOperator(

    task_id='dwd_oride_finance_driver_repayment_extend_test_df_task',
    hql='''
  ...
  (SELECT driver_id,
   ...
   FROM
     (SELECT driver_id,
      ...
      FROM oride_dw_ods.ods_sqoop_base_data_driver_balance_extend_df
      WHERE dt BETWEEN DATE_ADD('{pt}', -16) AND DATE_ADD('{pt}', -1)
      ORDER BY driver_id,
...
ods_sqoop_base_data_driver_repayment_df_prev_day_tesk>>ods_sqoop_base_data_driver_balance_extend_df_prev_day_tesk>>dim_oride_driver_base_prev_day_task>>sleep_time>>dwd_oride_finance_driver_repayment_extend_test_df_task>>task_check_key_data>>touchz_data_success