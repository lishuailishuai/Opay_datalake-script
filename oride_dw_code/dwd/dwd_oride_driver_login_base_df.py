...
dag = airflow.DAG( 'dwd_oride_driver_login_base_df', 
...
table_name="dwd_oride_driver_login_base_df"
hdfs_path="ufile://opay-datalake/oride/oride_dw/"+table_name

##----------------------------------------- 脚本 ---------------------------------------## 


dwd_oride_driver_login_base_df_task = HiveOperator(

    task_id='dwd_oride_driver_login_base_df_task',
    hql='''
...
def check_key_data(ds,**kargs):
    ...
    #主键重复校验
    HQL_DQC='''
    SELECT count(1)-count(distinct driver_id,device_id) as cnt
      FROM oride_dw.{table}
      WHERE dt='{pt}'
    '''.format(
        pt=ds,
...
dwd_oride_driver_login_base_df_prev_day_tesk>>sleep_time>>dwd_oride_driver_login_base_df_task>>task_check_key_data>>touchz_data_success
