...
dag = airflow.DAG('dwd_oride_driver_hf',
...
db_name = "oride_dw"
table_name = "dwd_oride_driver_hf"
hdfs_path = "oss://opay-datalake/oride/oride_dw/" + table_name
##----------------------------------------- 依赖 ---------------------------------------##

ods_binlog_base_data_driver_h_his_prev_day_task = OssSensor(
    task_id='ods_binlog_base_data_driver_h_his_prev_day_task',
    bucket_key='{hdfs_path_str}/dt={pt}/hour={now_hour}/_SUCCESS'.format(
        hdfs_path_str="oride_h_his/ods_binlog_base_data_driver_h_his",
        pt='{{ds}}',
...
def dwd_oride_driver_hf_sql_task(ds, hour):
    HQL = '''
        ...
        select
          ...
          plate_number,--'车牌号',
          real_name,--'真实姓名',
          from_unixtime(unix_timestamp(birthday,'yyyy-MM-dd')+3600,'yyyy-MM-dd'),--生日
          gender,--'性别',
          ...
          momo_isp,--'运营商1、Vodafone 2、MTN 3、Airtel & Tigo',
          momo_account,--'运营商账号'
          'nal' as country_code,
          '{pt}' as dt,
          hour
          ...
          from
          oride_dw_ods.ods_binlog_base_data_driver_h_his t
...
def execution_data_task_id(ds, **kwargs):
    ...
    # 读取sql
    _sql = dwd_oride_driver_hf_sql_task(ds, v_hour)
...
dwd_oride_driver_hf_task = PythonOperator(
...
ods_binlog_base_data_driver_h_his_prev_day_task >> dwd_oride_driver_hf_task
