...
dependence_server_magic_dispatch_detail_prev_day_tesk=HivePartitionSensor(
      task_id="server_magic_dispatch_detail_prev_day_tesk",
...
select 
      ord.city_id,
       ...
       count(ord.order_id)-count(distinct p1.order_id) as succ_broadcast_cnt,--成功播单数
...
FROM oride_dw.dwd_oride_order_push_driver_detail_di
   WHERE dt='{pt}'
     AND success=1
   GROUP BY order_id) p1 ON ord.order_id=p1.order_id
LEFT OUTER JOIN
  (SELECT order_id,
          sum(distance) AS pick_up_distance --接驾距离
FROM oride_dw.dwd_oride_order_assign_driver_detail_di
   WHERE dt='{pt}'
   GROUP BY order_id) a1 ON ord.order_id=a1.order_id
LEFT OUTER JOIN
  (SELECT order_id
   FROM oride_bi.server_magic_dispatch_detail --播单
   WHERE dt='{pt}'
   GROUP BY order_id) d1 ON ord.order_id=d1.order_id
...
dependence_dwd_oride_order_base_include_test_di_prev_day_tesk>>dependence_dwd_oride_order_assign_driver_detail_di_prev_day_tesk>>dependence_dwd_oride_order_push_driver_detail_di_prev_day_tesk>>dependence_server_magic_dispatch_detail_prev_day_tesk>>sleep_time>>dm_oride_order_base_d_task>>touchz_data_success
