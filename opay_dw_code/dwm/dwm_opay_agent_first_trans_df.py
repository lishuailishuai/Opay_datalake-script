...
dag = airflow.DAG('dwm_opay_agent_first_trans_df',
...
table_name = "dwm_opay_agent_first_trans_df"
hdfs_path = "oss://opay-datalake/opay/opay_dw/" + table_name


def dwm_opay_agent_first_trans_df_sql_task(ds):
    HQL = '''
    
    set mapred.max.split.size=1000000;
    set hive.exec.dynamic.partition.mode=nonstrict;
    set hive.exec.parallel=true;
    with agent_first_trans_di as (
        select 
        ...
        from (
            ...
            where dt = '{pt}' and user_role = 'agent'
        ) t0 where rn = 1
    )
    ...
    select 
        ...
        user_id, originator_or_affiliate,
        top_consume_scenario, sub_consume_scenario,
        country_code,
        '{pt}'
    from opay_dw.dwm_opay_agent_first_trans_df where dt = date_sub('{pt}', 1)
    union all
    select 
        order_no, trans_time, amount, currency, 
        top_service_type, sub_service_type, 
        t0.user_id, originator_or_affiliate,
...
def execution_data_task_id(ds, **kargs):
    ...
    # 读取sql
    _sql = dwm_opay_agent_first_trans_df_sql_task(ds)
...
dwm_opay_agent_first_trans_df_task = PythonOperator(
    task_id='dwm_opay_agent_first_trans_df_task',
    python_callable=execution_data_task_id,
    provide_context=True,
    dag=dag
)

dwd_opay_user_transaction_record_di_prev_day_task >> dwm_opay_agent_first_trans_df_task
