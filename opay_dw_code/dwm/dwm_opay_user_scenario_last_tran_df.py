...
dwm_opay_user_scenario_last_tran_df_prev_day_task = OssSensor(
    task_id='dwm_opay_user_scenario_last_tran_df_prev_day_task',
    bucket_key='{hdfs_path_str}/dt={pt}/_SUCCESS'.format(
        hdfs_path_str="opay/opay_dw/dwm_opay_user_scenario_last_tran_df/country_code=NG",
...
table_name = "dwm_opay_user_scenario_last_tran_df"
hdfs_path = "oss://opay-datalake/opay/opay_dw/" + table_name


def dwm_opay_user_scenario_last_tran_df_sql_task(ds):
    HQL = '''
    ...
    with tran as 
     ...
     from
       (select m.* 
        from 
           (select a.*,row_number()over(partition by user_id,flag,top_consume_scenario order by create_time desc) rn 
             from tran a) m 
        where rn=1
       ) a 
     full join 
       (select * from opay_dw.dwm_opay_user_scenario_last_tran_df where dt='{yesterday}') b 
     on a.user_id=b.user_id and a.flag=b.flag and a.top_consume_scenario=b.top_consume_scenario
...
def execution_data_task_id(ds, **kargs):
    ...
    # 读取sql
    _sql = dwm_opay_user_scenario_last_tran_df_sql_task(ds)
...
dwm_opay_user_scenario_last_tran_df_task = PythonOperator(
    task_id='dwm_opay_user_scenario_last_tran_df_task',
    python_callable=execution_data_task_id,
    provide_context=True,
    dag=dag
)

dwd_opay_transaction_record_di_prev_day_task >> dwm_opay_user_scenario_last_tran_df_task
dwm_opay_user_scenario_last_tran_df_prev_day_task >> dwm_opay_user_scenario_last_tran_df_task
