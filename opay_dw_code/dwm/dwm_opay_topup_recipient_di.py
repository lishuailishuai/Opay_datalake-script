...
dependence_dwd_opay_merchant_topup_record_di_prev_day_task = UFileSensor(
    ...
    filepath='{hdfs_path_str}/dt={pt}/_SUCCESS'.format(
        hdfs_path_str="opay/opay_dw/dwd_opay_merchant_topup_record_di",
        pt='{{ds}}'
    ),
...
dependence_dwd_opay_user_topup_record_di_prev_day_task = UFileSensor(
    ...
    filepath='{hdfs_path_str}/dt={pt}/_SUCCESS'.format(
        hdfs_path_str="opay/opay_dw/dwd_opay_user_topup_record_di",
        pt='{{ds}}'
    ),
    bucket_name='opay-datalake',
    poke_interval=60,  # 依赖不满足时，一分钟检查一次依赖状态
...
from 
    (select merchant_id recipient_id,'MERCHANT' recipient_type,'merchant' recipient_role,'TopupWithCard' service_type,order_status,amount,country,dt from dwd_opay_merchant_topup_record_di
     where dt='{pt}'
      union all
    select user_id recipient_id,'USER' recipient_type,user_role recipient_role,'TopupWithCard' service_type,order_status,amount,country,dt from dwd_opay_user_topup_record_di
     where dt='{pt}')m
