...
dag = airflow.DAG('dwd_opay_merchant_transfer_user_record_di',
...
##----------------------------------------- 变量 ---------------------------------------##
db_name = "opay_dw"
table_name = "dwd_opay_merchant_transfer_user_record_di"
hdfs_path="ufile://opay-datalake/opay/opay_dw/" + table_name


##---- hive operator ---##
# fill_dwd_opay_merchant_transfer_user_record_di_task = HiveOperator(
#     task_id='fill_dwd_opay_merchant_transfer_user_record_di_task',
#     hql='''
...
#             end as country_code,
#         order_di.dt
#     from opay_dw_ods.ods_sqoop_base_merchant_transfer_user_record_di order_di
...
##---- hive operator ---##
def dwd_opay_merchant_transfer_user_record_di_sql_task(ds):
    HQL='''
    set hive.exec.dynamic.partition.mode=nonstrict;
    with user_data as(
        select * from
    ...
    (
        select 
            ...
            dt
        from
        opay_dw_ods.ods_sqoop_base_merchant_transfer_user_record_di 
        where dt='{pt}'
    ) order_di 
    left join user_data recipient_di on recipient_di.user_id = order_di.recipient_id
...
def execution_data_task_id(ds, **kargs):
    ...
    # 读取sql
    _sql = dwd_opay_merchant_transfer_user_record_di_sql_task(ds)
...
dwd_opay_merchant_transfer_user_record_di_task = PythonOperator(
