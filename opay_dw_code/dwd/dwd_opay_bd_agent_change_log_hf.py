...
dag = airflow.DAG('dwd_opay_bd_agent_change_log_hf',
...
##----------------------------------------- 变量 ---------------------------------------##
db_name = "opay_dw"
table_name = "dwd_opay_bd_agent_change_log_hf"
...
##----------------------------------------- 依赖 ---------------------------------------##
### 检查上一个小时的本地时间依赖
dwd_opay_bd_agent_change_log_hf_pre_locale_task = OssSensor(
    task_id='dwd_opay_bd_agent_change_log_hf_pre_locale_task',
...
def dwd_opay_bd_agent_change_log_hf_sql_task(ds, v_date):
    HQL = '''
    set hive.exec.dynamic.partition.mode=nonstrict;
    set hive.exec.parallel=true;
    
    with ods_bd_agent_change_log_hi as (
        SELECT 
            id, agent_id as bd_agent_id, 
            cast(opay_id as string) as user_id, 
            created_at as create_time,
            from_agent_status, to_agent_status,  
            row_number() over(partition by agent_id order by `__ts_ms` desc,`__file` desc,cast(`__pos` as int) desc) rn
    ...
    from (
        select 
            ...
            to_agent_status,  
            country_code,
            row_number() over(partition by t1.id order by create_time desc) rn
        from (
            SELECT 
                id, 
                bd_agent_id, 
            ...
            SELECT 
                ...
                to_agent_status,     
                'NG' AS country_code
            from ods_bd_agent_change_log_hi 
            where rn = 1
        )t1 left join (
            select 
                id, bd_admin_user_id
...
def execution_data_task_id(ds, dag, **kwargs):
    ...
    # 读取sql
    _sql="\n"+cf.alter_partition()+"\n"+dwd_opay_bd_agent_change_log_hf_sql_task(ds, v_date)
...
dwd_opay_bd_agent_change_log_hf_task = PythonOperator(
    task_id='dwd_opay_bd_agent_change_log_hf_task',
...
dwd_opay_bd_agent_change_log_hf_pre_locale_task >> dwd_opay_bd_agent_change_log_hf_task
ods_bd_agent_status_change_log_hi_check_task >> dwd_opay_bd_agent_change_log_hf_task
dim_opay_bd_agent_hf_check_task >> dwd_opay_bd_agent_change_log_hf_task
