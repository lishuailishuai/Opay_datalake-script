...
##----------------------------------------- 变量 ---------------------------------------##
db_name="opay_dw"
table_name="dm_opay_transfer_of_account_relation_d"
hdfs_path="ufile://opay-datalake/opay/opay_dw/"+table_name

##---- hive operator ---##
def dm_opay_transfer_of_account_relation_d_sql_task(ds):
    HQL='''
    ...
    from (
        select 
            ...
            amount
        from {db}.dwd_opay_transfer_of_account_record_di
        where dt = '{pt}' 
            and create_time BETWEEN date_format(date_sub('{pt}', 1), 'yyyy-MM-dd 23') AND date_format('{pt}', 'yyyy-MM-dd 23')
    ) t1
    group by country_code, sub_service_type, payment_relation_id, order_status
    with cube
    '''.format(
        pt=ds,
        table=table_name,
...
def execution_data_task_id(ds, **kargs):
    ...
    # 读取sql
    _sql = dm_opay_transfer_of_account_relation_d_sql_task(ds)
...
dwd_opay_transfer_of_account_record_di_prev_day_task >> dm_opay_transfer_of_account_relation_d_task