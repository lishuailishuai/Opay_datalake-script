...
dag = airflow.DAG('app_opay_bd_agent_pos_ng_h',
...
def app_opay_bd_agent_pos_ng_h_sql_task(ds, v_date):
    HQL = '''
    ...
    select
        t1.create_date,
        t1.bd_admin_user_id,
        t2.bd_admin_user_name,
        t2.bd_admin_job_id,
        t1.originator_id as user_id,
        t1.pos_order_cnt,
        t1.pos_order_amt,
        t2.job_bd_user_id, 
        t2.job_bdm_user_id, 
    ...
    from (
        select 
            create_date, bd_admin_user_id, originator_id, 
            count(*) as pos_order_cnt,
            sum(amount) as pos_order_amt
        from (
            select 
                bd_admin_user_id, originator_id, 
        ...
        where country_code = 'NG' and concat(dt,' ',hour) = date_format(default.localTime("{config}", 'NG', '{v_date}', 0), 'yyyy-MM-dd HH')
...
def execution_data_task_id(ds, dag, **kwargs):
    ...
    # 读取sql
    _sql = "\n" + cf.alter_partition() + "\n" + app_opay_bd_agent_pos_ng_h_sql_task(ds, v_date)
...
app_opay_bd_agent_pos_ng_h_task = PythonOperator(
    task_id='app_opay_bd_agent_pos_ng_h_task',
...
dwd_opay_pos_transaction_record_hi_check_task >> app_opay_bd_agent_pos_ng_h_task
