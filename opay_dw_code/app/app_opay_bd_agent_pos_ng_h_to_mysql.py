...
dag = airflow.DAG(
    'app_opay_bd_agent_pos_ng_h_to_mysql',
...
##----------------------------------- 变量 ----------------------------------##
db_name = "opay_dw"
table_name = "app_opay_bd_agent_pos_ng_h"
hdfs_path = "oss://opay-datalake/opay/opay_dw/" + table_name
config = eval(Variable.get("opay_time_zone_config"))
time_zone = config['NG']['time_zone']
mysql_table = 'opay_dw.app_opay_bd_agent_pos_ng_h'
...
### 检查当前小时的依赖
app_opay_bd_agent_pos_ng_h_check_task = OssSensor(
    task_id='app_opay_bd_agent_pos_ng_h_check_task',
    bucket_key='{hdfs_path_str}/country_code=NG/dt={pt}/hour={hour}/_SUCCESS'.format(
        hdfs_path_str="opay/opay_dw/app_opay_bd_agent_pos_ng_h",
...
### 检查上一个小时的依赖
app_opay_bd_agent_pos_ng_h_pre_check_task = OssSensor(
    task_id='app_opay_bd_agent_pos_ng_h_pre_check_task',
    bucket_key='{hdfs_path_str}/country_code=NG/dt={pt}/hour={hour}/_SUCCESS'.format(
        hdfs_path_str="opay/opay_dw/app_opay_bd_agent_pos_ng_h",
        pt='{{{{(execution_date+macros.timedelta(hours=({time_zone}+{gap_hour}))).strftime("%Y-%m-%d")}}}}'.format(
...
def get_data_from_hive(ds, execution_date,  **op_kwargs):
    ...
    hql = '''
        SELECT 
            ...
            dt,
            hour
        from opay_dw.app_opay_bd_agent_pos_ng_h
        where 
        country_code = 'NG' 
        --当前小时
...
def __data_to_mysql(conn, data, column):
    isql = 'replace into {table} ({columns})'.format(
        table=mysql_table,
        columns=','.join(column)
    )
    esql = '{0} values {1}'
    ...
    cnt = 0
    try:
        for (create_date,bd_admin_user_id,bd_admin_user_name,bd_admin_job_id,user_id,
            pos_order_cnt,pos_order_amt,job_bd_user_id,job_bdm_user_id,job_rm_user_id,job_cm_user_id,job_hcm_user_id,job_pic_user_id,country_code,dt,hour
            ) in data:

            row = [create_date,bd_admin_user_id,bd_admin_user_name,bd_admin_job_id,user_id,
                    pos_order_cnt,pos_order_amt,job_bd_user_id,job_bdm_user_id,job_rm_user_id,
                    job_cm_user_id,job_hcm_user_id,job_pic_user_id,country_code,dt,hour]
            if sval == '':
...
app_opay_bd_agent_pos_ng_h_check_task >> get_data_from_hive_task
app_opay_bd_agent_pos_ng_h_pre_check_task >> get_data_from_hive_task