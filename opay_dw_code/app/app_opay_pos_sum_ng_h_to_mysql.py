...
dag = airflow.DAG(
    'app_opay_pos_sum_ng_h_to_mysql',
...
mysql_table = 'opay_dw.app_opay_pos_sum_ng_h'

##----------------------------------依赖数据源------------------------------##


### 检查当前小时的依赖
app_opay_pos_sum_ng_h_check_task = OssSensor(
    task_id='app_opay_pos_sum_ng_h_check_task',
...
### 检查上一个小时的依赖
app_opay_pos_sum_ng_h_pre_check_task = OssSensor(
    task_id='app_opay_pos_sum_ng_h_pre_check_task',
    bucket_key='{hdfs_path_str}/country_code=NG/dt={pt}/hour={hour}/_SUCCESS'.format(
        hdfs_path_str="opay/opay_dw/app_opay_pos_sum_ng_h",
        pt='{{{{(execution_date+macros.timedelta(hours=({time_zone}+{gap_hour}))).strftime("%Y-%m-%d")}}}}'.format(
...
def get_data_from_hive(ds, execution_date, **op_kwargs):
    ...
    hql = '''
        ...
        from  oride_dw.app_opay_pos_sum_ng_h
        where 
        country_code = 'NG'
...
app_opay_pos_sum_ng_h_check_task >> get_data_from_hive_task
app_opay_pos_sum_ng_h_pre_check_task >> get_data_from_hive_task