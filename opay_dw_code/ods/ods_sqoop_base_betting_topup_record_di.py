...
dag = airflow.DAG('ods_sqoop_base_betting_topup_record_di',
...
ods_binlog_base_betting_topup_record_check_hi_task = OssSensor(
    task_id='ods_binlog_base_betting_topup_record_check_hi_task',
...
table_name = "ods_sqoop_base_betting_topup_record_di"
hdfs_path = "oss://opay-datalake/opay_dw_sqoop_di/opay_transaction/betting_topup_record"
config = eval(Variable.get("opay_time_zone_config"))


def ods_sqoop_base_betting_topup_record_di_sql_task(ds):
    HQL = '''
    ...
    from 
        (select *,row_number() over(partition by id order by `__ts_ms` desc,`__file` desc,cast(`__pos` as int) desc) rn
         FROM opay_dw_ods.ods_binlog_base_betting_topup_record_hi
...
def execution_data_task_id(ds, **kargs):
    ...
    # 读取sql
    _sql = ods_sqoop_base_betting_topup_record_di_sql_task(ds)
...
ods_sqoop_base_betting_topup_record_di_task = PythonOperator(
    task_id='ods_sqoop_base_betting_topup_record_di_task',
    python_callable=execution_data_task_id,
    provide_context=True,
    dag=dag
)

ods_binlog_base_betting_topup_record_check_hi_task >> ods_sqoop_base_betting_topup_record_di_task
