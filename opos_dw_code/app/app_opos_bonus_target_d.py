...
sum(if(discount_type is null and length(discount_ids)>0,nvl(org_payment_amount,0),0))/count(distinct(if(discount_type is null and length(discount_ids)>0,sender_id,null))) 
  - sum(if(discount_type is null and length(discount_ids)=0,nvl(org_payment_amount,0),0))/count(distinct(if(discount_type is null and length(discount_ids)=0,sender_id,null))) as people_avg_amt_increase,
--订单均消费金额增幅（红包用户）,用红包支付的单均应付金额-没用红包支付的单均应付金额,这个分母不需要加distinct,
sum(if(discount_type is null and length(discount_ids)>0,nvl(org_payment_amount,0),0))/count(if(discount_type is null and length(discount_ids)>0,sender_id,null)) 
  - sum(if(discount_type is null and length(discount_ids)=0,nvl(org_payment_amount,0),0))/count(if(discount_type is null and length(discount_ids)=0,sender_id,null)) as order_avg_amt_increase,
--单均实付增幅（红包用户）,用实付
sum(if(discount_type is null and length(discount_ids)>0,nvl(pay_amount,0),0))/count(if(discount_type is null and length(discount_ids)>0,sender_id,null)) 
