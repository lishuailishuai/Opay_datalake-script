...
app_opos_shop_target_month_m_task = OssSensor(
    task_id='app_opos_shop_target_month_m_task',
    bucket_key='{hdfs_path_str}/country_code=nal/dt={pt}/_SUCCESS'.format(
        hdfs_path_str="opos/opos_dw/app_opos_shop_target_month_m",
        pt='{{ds}}'
    ),
...
if '{{ds}}'[-2:] == '01':
    delete_sql = """
DELETE FROM opos_dw.app_opos_shop_target_month_m WHERE dt='{ds}';
--DELETE FROM opos_dw.app_opos_shop_target_month_m WHERE dt='{before_1_day}';
            """.format(
        ds='{{ds}}',
...
,dt
from
opos_dw.app_opos_shop_target_month_m
where
country_code = 'nal'
and dt='{ds}'
    ...
    mysql_conn_id='mysql_dw',
    mysql_table='app_opos_shop_target_month_m',
    dag=dag)

##----------------------------------------- 最后执行流程 ---------------------------------------##

app_opos_shop_target_month_m_task >> drop_mysql_yesterday_data >> insert_mysql_today_data
