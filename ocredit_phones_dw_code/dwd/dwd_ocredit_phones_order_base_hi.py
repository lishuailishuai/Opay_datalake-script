...
def dwd_ocredit_phones_order_base_hi_sql_task(ds, v_date):
    ...
    select id, --无业务含义主键 
          ...
          default.localTime("{config}",'NG',unix_timestamp(create_time/1000),0) as create_time, --创建时间 
          default.localTime("{config}",'NG',unix_timestamp(update_time/1000),0) as update_time, --更新时间 
          default.localTime("{config}",'NG',unix_timestamp(loan_time/1000),0) as loan_time, --放款时间 
          opr_id, --操作更新用户ID 
          ...
          if(order_id='012020011001240073','2020-01-04',substr(default.localTime("{config}",'NG',unix_timestamp(create_time/1000),0),1,10)) as date_of_entry, --进件日期
        t1.utc_date_hour,
