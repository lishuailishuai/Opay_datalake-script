...
dag = airflow.DAG('dwd_ocredit_overdue_config_hf',
...
table_name = "dwd_ocredit_overdue_config_hf"
...
### 检查当前小时的分区依赖
ods_binlog_base_t_overdue_config_h_his_check_task = OssSensor(
    task_id='ods_binlog_base_t_overdue_config_h_his_check_task',
    bucket_key='{hdfs_path_str}/dt={pt}/hour={hour}/_SUCCESS'.format(
        hdfs_path_str="ocredit_phones_h_his/ods_binlog_base_t_overdue_config_h_his",
        pt='{{ds}}',
...
def dwd_ocredit_overdue_config_hf_sql_task(ds, v_date):
    HQL = '''
    ...
    select id,                    -- 主键              
            f_product_id,          -- 金融产品id          
            business_type,         -- 业务类型            
            min_overdue_day_num,   -- 最小逾期天数          
            max_overdue_day_num,   -- 最大逾期天数          
            overdue_interest_rate, -- 逾期天数适用费率        
            version,               -- 版本号             
    ...
    from (select *,
             ...
             from ocredit_phones_dw_ods.ods_binlog_base_t_overdue_config_h_his
            where 
...
def execution_data_task_id(ds, dag, **kwargs):
    ...
    # 读取sql
    _sql = "\n" + cf.alter_partition() + "\n" + dwd_ocredit_overdue_config_hf_sql_task(ds, v_date)
...
dwd_ocredit_overdue_config_hf_task = PythonOperator(
    task_id='dwd_ocredit_overdue_config_hf_task',
...
ods_binlog_base_t_overdue_config_h_his_check_task >> dwd_ocredit_overdue_config_hf_task
