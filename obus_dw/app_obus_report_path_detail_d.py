...
dependence_ods_sqoop_conf_city_df = S3PrefixSensor(
    task_id='dependence_ods_sqoop_conf_city_df',
    prefix='obus_dw/ods_sqoop_conf_city_df/country_code=nal/dt={pt}'.format(pt='{{ ds }}'),
    bucket_name='opay-bi',
    dag=dag
)
...
def get_data_from_impala(**op_kwargs):
    ...
    sql = '''
        ...
        select 
            ...
            nvl(dc.name, ''),
            nvl(station_data.line_id, 0),
            nvl(line_data.name, ''),
            station_data.id,
            station_data.name,
            nvl(driver_data.total_drivers, 0),
            nvl(driver_data.serv_drivers, 0),
            nvl(driver_data.serv_on_the_road_drivers, 0),
            nvl(driver_data.serv_idle_drivers, 0),
            nvl(driver_data.no_serv_drivers, 0),
            nvl(line_order_data.lines_orders, 0),
            nvl(station_order_data.station_orders, 0),
            nvl(line_order_data.lines_finished_orders, 0),
            nvl(station_order_data.station_finished_orders, 0),
            nvl(new_users_data.new_users, 0),
            nvl(station_order_data.get_on_users, 0),
            nvl(get_off_users.get_off_users, 0),
            nvl(line_order_data.line_gmv_single, 0)
        from station_data 
