...
dag = airflow.DAG(
    'ocredit_phones_source_binlog_h_his',
...
dag_monitor = airflow.DAG(
    'ocredit_phones_source_binlog_h_his_monitor',
    schedule_interval=schedule_interval,
    default_args=args)
    ...
    if len(cursor.fetchall()) == 0:
        ...
        for result in results:
            if result[0] == 'dt':
                ...
                col_name = result[0]
            if result[1] == 'timestamp' or result[1] == 'varchar' or result[1] == 'char' or result[1] == 'text' or \
                    result[1] == 'datetime' or result[1] == 'mediumtext' or result[1] == 'json':
                data_type = 'string'
            elif result[1] == 'decimal':
                data_type = result[1] + "(" + str(result[2]) + "," + str(result[3]) + ")"
            else:
