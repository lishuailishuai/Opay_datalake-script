...
dag = airflow.DAG('dwm_oride_driver_finance_di_dev',
...
def dwm_oride_driver_finance_di_sql_task(ds):
    ...
    select dri.driver_id,
        ...
        where dt='{pt}' and from_unixtime((day+1*60*60),'yyyy-MM-dd')='{pt}'
        ) day
        ...
        where dt='{pt}'
        and from_unixtime((created_at+1*60*60),'yyyy-MM-dd')='{pt}'
        ...
        where dt='{pt}'
        and from_unixtime((create_time+1*60*60),'yyyy-MM-dd')='{pt}'  --后续该限制条件要去除
        group by driver_id
        ) reward
        ...
        AND pay.record_day=(day.day+1*60*60)
        LEFT JOIN
          (SELECT driver_id,
                  from_unixtime((created_at+1*60*60),'yyyy-MM-dd') as create_date,
           ...
           WHERE dt='{pt}'
             AND amount_reason IN(4,
                                  ...
                                  7)
           group by driver_id,
                  from_unixtime((created_at+1*60*60),'yyyy-MM-dd')) recharge ON day.driver_id=recharge.driver_id
        AND from_unixtime((day.day+1*60*60),'yyyy-MM-dd')=recharge.create_date
        ...
        (
            ...
            (
                select driver_id,
                    from_unixtime(min((created_at+1*60*60)),'yyyy-MM-dd') as start_date
                from oride_dw.dwd_oride_driver_recharge_records_df 
                where dt='{pt}'
                and amount_reason=6 
                and amount<0 
