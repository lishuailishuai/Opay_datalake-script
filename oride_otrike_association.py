...
create_oride_otrike_association_di = HiveOperator(
    task_id='create_oride_otrike_association_di',
    hql="""
        CREATE TABLE IF NOT EXISTS oride_otrike_association_di (
...
insert_oride_otrike_association_di = HiveOperator(
    task_id='insert_oride_otrike_association_di',
    hql="""
        -- 协会数据
        ...
        INSERT OVERWRITE TABLE oride_otrike_association_di PARTITION (dt='{{ ds }}')
        SELECT
            bd.association_id,
...
def send_oride_association_email(ds, **kwargs):
    ...
    query = '''
        ...
        with t_d as (
              ...
              *
            FROM
              oride_bi.oride_otrike_association_di
            WHERE
              dt='{dt}'
        ),
        -- 昨日数据
    ...
    file_name = '/tmp/oride_otrike_association_{dt}.csv'.format(dt=ds)
...
for table_name in table_names:
    oride_association_validate_task = HivePartitionSensor(
        task_id="oride_association_validate_task_{}_{}".format(*table_name.split('.')),
        table=table_name,
        partition="dt='{{ds}}'",
        poke_interval=60,  # 依赖不满足时，一分钟检查一次依赖状态
        dag=dag
    )
    oride_association_validate_task >> insert_oride_otrike_association_di

create_oride_otrike_association_di >> insert_oride_otrike_association_di >> oride_association_email
